...
MATCH_ID  = 153341   # TEST: PAK W vs ZIM W (4 PM) | RR vs GT ke liye: 152075
...
def tg(text):
    try:
        requests.post(TG_URL, json={"chat_id": CHAT_ID, "text": text, "parse_mode": "HTML"}, timeout=5)
...
def main():
    ...
    tg(
        ...
        f"⚡ Polling every 500ms — ultra fast"
    )
    ...
    while True:
        ...
        if loop % 60 == 0:
            print(f"[{now()}] state={state} | toss={winner or 'waiting'} | checks={loop}")
